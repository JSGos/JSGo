...
2. Run the modified restler in /restler to generate API sequences following [this guide](https://github.com/microsoft/restler-fuzzer).
...
We also provide the running results of the example in this Reply phase stored in `targetAlighTraces_example`. If you are interested in the Mutate phase, you can skip all prior steps and directly use the files in `targetAlighTraces_example`. These are all what we need in the following mutation phase.
